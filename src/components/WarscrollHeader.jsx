...
function WarscrollHeader({ unit, color }) {
    let ward = null;
    if (unit.primaryKeywords && unit.primaryKeywords.join().toUpperCase().includes("WARD")) {
    ...
    return <div className="warscroll-header" >
        ...
        <div className="stat-card">
            ...
            {ward ? <div className="stat">{unit.stats.save}/{ward}</div> : <div className="stat">{unit.stats.save}</div>}
        </div>
        {unit.stats.control && <div className="stat-card">
            <div className="stat-header">Control</div>
            <div className="stat">{unit.stats.control}</div>
        </div>}
        {unit.stats.banishment && <div className="stat-card">
